...
public interface SeedObfuscationStrategy {
	...
	static PacketContainer obfuscate(PacketContainer packet, long hash) {
		if (packet.getType() == PacketType.Play.Server.LOGIN) {
			return DYNAMIC_LOGIN.process(packet, hash);
		} else if (packet.getType() == PacketType.Play.Server.RESPAWN) {
			return DYNAMIC_RESPAWN.process(packet, hash);
		}
		return packet;
	}
