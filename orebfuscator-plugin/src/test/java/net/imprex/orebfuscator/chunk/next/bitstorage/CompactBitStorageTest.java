...
public class CompactBitStorageTest extends AbstractBitStorageTest {

	private static final byte[] IDENTITY_BUFFER = readIdentityBuffer("src/test/resources/compact-bit-storage.bin");
	...
	public void testReadWriteIdentity() {
		...
		BitStorage bitStorage = CompactBitStorage.read(buffer, INDENTITY_BUFFER_ENTRIES, palette);
		...
		BitStorage.Writer writer = new CompactBitStorage.Writer(writeBuffer, INDENTITY_BUFFER_ENTRIES, palette);
		for (int i = 0; i < INDENTITY_BUFFER_ENTRIES; i++) {
			writer.write(i);
		}
